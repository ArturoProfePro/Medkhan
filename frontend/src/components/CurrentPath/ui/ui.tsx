...
const pathsIdentify: Record<string, {
    title: string;
    path: string;
}> = {
    'services': {
...
const CurrentPath = () => {
    ...
    return (
        <div className={c.container}>
            <Link to="/" className={c.main}>Главная</Link>
            {paths.map(path => <>
                <span> </span>/<Link to={pathsIdentify[path].path} className={c.path}> {pathsIdentify[path].title}</Link>
            </>)}
        </div>
    )
}
